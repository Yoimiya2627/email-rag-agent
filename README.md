...
2. **LLM Reranker 是 trade-off 不是单边负贡献**：V2→V3 让 context_precision +0.025（在做它该做的事——剔噪声），代价是 relevancy -0.029；要进一步优化建议替换为 cross-encoder（如 bge-reranker-v2-m3），LLM 当 reranker 在该数据集下收益不稳定，且 V2→V3 mean 延迟增加约 12s（cross-encoder 预计可压到毫秒级）
...
**业务选型**：在邮件查询场景（关心 relevancy + 低延迟）最终选 V2 方案——在该数据集上 answer_relevancy 优于全开 V4 配置，V2 延迟约为 V4 的 1/3（mean 8.7s vs 24.2s，p95 14.4s vs 38s，10 题/版 trimmed mean）。组件不是越多越好，按业务目标和延迟要求选方案。

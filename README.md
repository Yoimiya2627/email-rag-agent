...
**业务选型**：在邮件查询场景（关心 relevancy + 低延迟）最终选 V2 方案——在该数据集上 answer_relevancy 优于全开 V4 配置，端到端延迟低 ~3 倍（mean 8.7s vs 24.2s，p95 14.4s vs 38s，10 题/版 trimmed mean）。组件不是越多越好，按业务目标和延迟要求选方案。

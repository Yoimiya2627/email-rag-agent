.PHONY: help install preload index api ui run mcp eval agent-eval trace-summary eval-all latency reranker-latency gmail-sync context-recall test clean
...
help:
	...
	@echo "  make preload     explicitly prepare/download the configured embedding model"
...
test:
	$(PYTHON) scripts/offline_tests.py tests/ -v

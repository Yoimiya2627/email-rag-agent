...
教训：Reranker **必须用 cross-encoder 模型**，用通用 LLM 当 reranker 是反模式。本项目这个选择是当时为了简化部署没引入新模型，付出了精度代价。
...
3. **LLM 当 Reranker 是反模式**——除非数据集特别复杂，否则用 cross-encoder。本项目下个版本会切到 bge-reranker-v2-m3。

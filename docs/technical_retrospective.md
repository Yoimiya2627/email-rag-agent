...
**现象**：原假设是"组件越全越好"——V4 全开版应该三维度全冠。实际跑 6 版消融后，**三个维度的赢家是分散的**，没有任何一版同时拿下三冠。

**进一步排查**：把这套消融**前后跑了两次**。两次都出现"三个指标被三个不同配置瓜分"——但**具体哪一版赢哪个指标变了**（第一次 V2/V4/V5，第二次 V6/V1/V3）。这个对比本身就是个发现：单次结果的精确排名并不可靠。

**根因**：分两层。（a）三个维度衡量 RAG 流水线的不同部位（生成是否切题 / 是否有据 / 检索是否干净），互相不存在"一个赢则其他都赢"的关系——所以"赢家分散"是**结构性的、稳定复现的**。（b）但精确排名会变，是因为**评测本身有噪声**：每版只抽 30 题（小样本），又用 LLM 当裁判（打分本身有波动），两者叠加足以翻转版本间 ±0.05~0.1 的差异。

**修复 / 业务取舍**：不存在"修"——评测教会我两件事。① **稳健的结论**："全开不是最优、组件之间是 trade-off"，把它翻译成业务取舍写进了 [`evaluation.md`](evaluation.md)：默认上线选 V2（relevancy 处于第一梯队，且不带 reranker/rewrite、延迟最低，mean 8.7s ≈ 全开 V4 的 1/3）。② **对评测本身的怀疑**：只声称"跨两次都成立的模式"，不声称精确排名；要更稳的结论得加大题量、多次取均值。
